...
async function setupDatabase() {
  ...
  // Try different PostgreSQL configurations with your password
  const configs = [
    {
      user: 'postgres',
      host: 'localhost',
  ...
  for (const config of configs) {
    try {
      console.log(`🔄 Trying to connect with user: ${config.user}, password: ${config.password ? '***' : 'none'}`);
  ...
  if (!adminPool) {
    ...
    console.log('');
    console.log('🔧 Please try these manual steps:');
    console.log('1. sudo -u postgres psql');
    console.log('2. ALTER USER postgres PASSWORD \'Cawet3Sewu\';');
    console.log('3. \\q');
  ...
  try {
    ...
    console.log('   3. Access: http://your-domain.com/admin');
